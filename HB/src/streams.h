...
public:
    ...
    {
        WriteShort(static_cast<uint16_t>(value));
    }
    ...
    void WriteString(const std::string & value, uint64_t length)
    {
        VerifyData;
        VerifySize(length);
...
public:
    ...
    {
        return static_cast<return_result>(ReadInt());
    }
    ...
    std::string ReadString(uint32_t sz)
    {
        VerifyData;
        VerifyLength(sz);
        char * c = new char[sz + 1];
